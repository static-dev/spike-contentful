...
test('errors without a "spaceId"', t => {
  t.throws(
    () => {
      new Contentful({ accessToken: 'xxx' })
    }, // eslint-disable-line
...
test.cb('writes json outputs', t => {
  ...
  project.on('compile', () => {
    ...
    const srcGlobal = JSON.parse(
      fs.readFileSync(path.join(projectPath, 'public/data.json'), 'utf8')
    )
    const srcSpecififc = JSON.parse(
      fs.readFileSync(path.join(projectPath, 'public/dogs.json'), 'utf8')
    )
...
test.cb('accepts template object and generates html', t => {
  ...
  project.on('compile', () => {
    const file1 = fs.readFileSync(
      path.join(projectPath, 'public/cats/Happy Cat.html'),
      'utf8'
    )
    const file2 = fs.readFileSync(
      path.join(projectPath, 'public/cats/Nyan Cat.html'),
      'utf8'
    )
